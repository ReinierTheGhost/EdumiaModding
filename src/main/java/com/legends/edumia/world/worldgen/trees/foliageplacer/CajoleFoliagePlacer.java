...
public class CajoleFoliagePlacer extends FoliagePlacer {
    public static final Codec<CajoleFoliagePlacer> CODEC = RecordCodecBuilder.create(instance ->
            fillFoliagePlacerFields(instance).apply(instance, CajoleFoliagePlacer::new));
    ...
    protected FoliagePlacerType<?> getType() {
        return EdumiaFoliagePlacerTypes.CAJOLE_FOLIAGE_PLACER;
    }
    ...
    protected void generate(TestableWorld world, BlockPlacer leaves, Random random, TreeFeatureConfig config, int trunkHeight, TreeNode foliage, int foliageHeight, int radius, int offset) {
        ...
        generateSquare(world, leaves, random, config, pos, 0, -2, flag);
        generateSquare(world, leaves, random, config, pos, radius + foliage.getFoliageRadius() - 1, -1, flag);
        generateSquare(world, leaves, random, config, pos, radius - 1, 0, flag);
        generateSquare(world, leaves, random, config, pos, radius + foliage.getFoliageRadius() - 1, 0, flag);
        generateSquare(world, leaves, random, config, pos, 0, 1, flag);
    }
