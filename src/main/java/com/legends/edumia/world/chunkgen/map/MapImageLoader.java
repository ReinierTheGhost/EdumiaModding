...
public class MapImageLoader {
    ...
    public static void loadImage(ClassLoader classLoader) throws IOException, URISyntaxException {
        URL resource = classLoader.getResource("assets/" + Edumia.MOD_ID + "/textures/edumia_biome_map.png");
        ...
        for (int i = 0; i < Edumia.MAP_ITERATION; i++) {
            subDivide(false);
        }
        ...
        EdumiaHeightMap.applyHeightMapImage(blurredImage);
    }
    ...
    public static float getBiomeHeight(int x, int y) {
        ...
        try {
            height = EdumiaBiomesData.biomeMap.get(pixels[y][x]).height;
        }
        catch (Exception e) {
            System.out.println("Unknown pixel color (" + color + ") at: " + x + "," + y + " at the Edumia map");
            height = EdumiaBiomesData.defaultBiome.height;
        }
        return height;
    }
    ...
    private static BufferedImage createHeightMap() {
        ...
        for (int y = 0; y < pixels.length; y++) {
            for (int x = 0; x < pixels[0].length; x++) {
                int height = (int) ((getBiomeHeight(x, y) - EdumiaBiomesData.MINIMAL_HEIGHT) * 4);
                try {
